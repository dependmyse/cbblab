...
else
{
    ...
    while($row = $dsql->GetObject())
    {
        ...
        /*************去除内容不匹配的链接****************/
        $matches=array();
        $matches =  explode("{dede:field name='body'}", $row->result);
        $matches = explode("{/dede:field}", $matches[1]);
        if(empty($matches[0]))
        {
            continue;
        }
        ...
        /***************处理图片的相对链接问题******************/
        $result=array();
        preg_match("{http://www.radi.cas.cn/dtxw/qysm/./(\\d{6})}", $row->url, $result);
        $row->result=str_replace('./',$result[1].'/',$row->result);
         /*********************************/
